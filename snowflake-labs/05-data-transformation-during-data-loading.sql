...
-- 5.0.0   Data Transformation During Data Loading
--         The purpose of this lab is to introduce you to how to transform data
--         when loading it.
--         - How to transform data during the loading process
...
--         as a region code. However, the file doesn’t have those two pieces of
...
-- 5.1.0   Transforming Data During Load

-- 5.1.1   Create a new folder called Data Loading and Transformation.

-- 5.1.2   Create a new worksheet named Transforming Data During Load.

-- 5.1.3   Create objects and set the context.


CREATE DATABASE IF NOT EXISTS HYENA_db;
USE DATABASE HYENA_db;
CREATE SCHEMA IF NOT EXISTS HYENA_transform;
USE SCHEMA HYENA_db.HYENA_transform;



-- 5.1.4   Search for the nation file.
...
-- 5.1.5   Query the file in the stage.
...
--         refer to columns in the SELECT clause in this way, whether or not
--         they exist in the file. If they don’t, the entire column will simply
--         be null.
...
-- 5.1.6   Create a pipe-delimited file format.
...
-- 5.1.8   Use the file format to query the file.
--         As you can see, we’ve added the file format right after the file
...
-- 5.1.9   Execute the statement below.
...
--         need. Let’s load the data now.

-- 5.1.10  Load the data.
...
--         In the result pane, you should see a row that says status = LOADED
--         and rows_loaded = 25. If you do, the load was successful.

-- 5.1.11  Run the SELECT statement below to view the contents of the NATION
--         table.
...
-- 5.1.12  Suspend and resize the virtual warehouse.
